...
import { isValidMeterId, isValidAmount } from "../utils/validation.js";
...
async function initPay({ username, amount }: { username: string; amount: number }): Promise<{ txn_identifier: any; req: string; sign: string }> {
...
function buildPayDisplayAddress(meterInfo: any): string {
  if (!meterInfo) return "";
...
function buildEnetsPayUrl({ req, sign, username, amount, address }: { req: string; sign: string; username: string; amount: number; address: string }): string {
...
async function fetchEnvJsp(): Promise<string | null> {
...
async function fetchNetsFields({ req, sign, username, amount, address }: { req: string; sign: string; username: string; amount: number; address: string }): Promise<{ txnReq: string; keyId: string; hmac: string; html: string }> {
...
async function callTxnReqListener({ txnReq, keyId, hmac }: { txnReq: string; keyId: string; hmac: string }): Promise<any> {
  let msgObj: unknown;
  try {
...
async function runBootstrap({ txtMtrId, txtAmount }: { txtMtrId: string; txtAmount: string }): Promise<any> {
...
async function callCreditInit({ txnRand, keyId, hmac, jsessionId }: { txnRand: string; keyId: string; hmac: string; jsessionId: string | null }): Promise<{ jsessionId: string | null; status: number }> {
...
}): Promise<{ message: string; hmac: string | null; keyId: string | null; action: string | null; preParsed: any }> {
...
async function postToB2s({ action, message, hmac, keyId, jsessionId }: { action: string | null; message: string; hmac: string | null; keyId: string | null; jsessionId: string | null }): Promise<{ status: number; html: string; parsed: any; finalUrl: string }> {
...
function parsePayResult(finalUrl: string, html: string): any {
  let r: string | null = null, t: string | null = null, a: string | null = null, x: string | null = null, s: string | null = null, m: string | null = null;

  try {
    const u = new URL(finalUrl);
    const b64dec = (v: string | null) =>
...
function renderFinalResultPage(parsed: any, basePath: string = BASE_PATH): string {
...
router.get("/webapp/bootstrap", async (req, res) => {
  ...
  try {
    ...
    track("bootstrap_failed", {
      meterId: txtMtrId,
      amount: txtAmount,
      stage: (err as any).stage || "unknown",
      error: (err as any).error || (err as Error).message,
    });
    return res.status(500).json({
      ok: false,
      stage: (err as any).stage || "unknown",
      step1Status: (err as any).step1Status,
      step2Status: (err as any).step2Status,
      step3Status: (err as any).step3Status,
      error: (err as any).error || (err as Error).message,
    });
  }
});
...
router.get("/webapp/pay", (req, res) => {
  ...
  const txnRand = String(req.query.txnRand || "");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      captureException(err, String((req.body as any)?.meterId || "anonymous"), {
        route: "cp2nus",
        merchantTxnRef: (req.body as any)?.merchantTxnRef || "",
      });
