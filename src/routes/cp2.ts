...
import { getMeterSummary } from "../services/ore.js";
import { isValidAmount, isValidMeterId } from "../utils/validation.js";
import { verifyInitData } from "../utils/initData.js";
import { config } from "../config.js";

const track = (..._args: unknown[]) => {};
const captureException = (..._args: unknown[]) => {};
...
function htmlDecode(str: unknown): string {
  return String(str || "")
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
...
function extractEvsCallbackFromHtml(html: unknown): { action: string; status: string | null; id: string | null; message: string } | null {
...
async function postResultToEvs({ status, id, message, jsessionid }: { status: string; id: string; message: string; jsessionid: string | null }): Promise<{ upstreamStatus: number; html: string; parsed: any }> {
...
function extractMerchantTxnRef(html: unknown): string | null {
  const body = String(html || "");
  const m =
    body.match(
...
function extractEnetsMessage(html: unknown): string | null {
  const body = String(html || "");
  const m = body.match(
...
function ensureBaseHref(html: unknown, baseHref: string): string {
  const body = String(html || "");
  if (!body) return body;
...
function resolveUpstreamLocation(baseUrl: string, location: unknown): string | null {
  try {
...
function normalizeFinalOutcome(parsed: any = {}): any {
...
function parseEvsTransactionSummary(html: unknown): any {
  const body = String(html || "");
...
function renderFinalResultPage(parsed: any): string {
...
async function getFollowRedirects(
  client: any,
  url: string,
  { params, headers, maxHops = 4 }: { params?: any; headers?: any; maxHops?: number } = {},
): Promise<any> {
...
function classifyLoginResponse(html: unknown): string {
  const body = String(html || "");
  const isValid =
...
function classifySelectOfferResponse(html: unknown): string {
  const body = String(html || "");
  const isSuccess =
...
function extractHiddenField(html: unknown, name: string): string | null {
...
function parseEnetsResult(html: unknown): any {
  const body = String(html || "");
...
async function createClient(): Promise<{ client: any; jar: CookieJar }> {
...
async function runPurchaseFlow({ txtMtrId, txtAmount }: { txtMtrId: string; txtAmount: string }): Promise<any> {
  const result: Record<string, any> = { ok: false, stage: "init" };
  ...
  if (loginResult !== "valid") {
    ...
    return {
      ...
      loginResult,
      cookieHeader: cookies.map((c: any) => `${c.key}=${c.value}`).join("; "),
    };
  }
...
router.get("/purchase_flow/enets", async (req, res) => {
  try {
    const out = await runPurchaseFlow({
      txtMtrId: String(req.query.txtMtrId || ""),
      txtAmount: String(req.query.txtAmount || ""),
    });
    ...
    return res.status(500).send(String((error as Error)?.message || error));
  }
});
...
router.get("/webapp/result", (req, res) => {
  ...
  const balance = String(req.query.balance || "");
...
router.get("/webapp/bootstrap", async (req, res) => {
  const txtMtrId = String(req.query.txtMtrId || "");
  const txtAmount = String(req.query.txtAmount || "");
  ...
  const initDataRaw = String(req.query.initData || "");
  if (!verifyInitData(initDataRaw, config.telegram.token)) {
...
router.get("/evs/merchant_txn_ref", async (req, res) => {
  try {
    const mode = String(req.query.mode || "0");
    const isDedicated = String(req.query.isDedicated || "1");
    const jsessionid = String(req.query.jsessionid || "");
...
router.get("/webapp/pay", (req, res) => {
  ...
  const netsMid = String(req.query.netsMid || "");
  const netsTxnRef = String(req.query.netsTxnRef || "");
  const merchantTxnRef = String(req.query.merchantTxnRef || "");
  const actionUrl = String(req.query.actionUrl || "");
...
router.post("/evs/creditpayment", async (req, res) => {
  try {
    const mode = String(req.body?.mode ?? "0");
    const isDedicated = String(req.body?.isDedicated ?? "1");
    const jsessionid = String(req.body?.jsessionid ?? "");
    const amt = String(req.body?.amt ?? "0.01");
    const payment_mode = String(req.body?.payment_mode ?? "CC");
    const txn_amount = String(req.body?.txn_amount ?? "1");
    const currency_code = String(req.body?.currency_code ?? "SGD");
    const submission_mode = String(req.body?.submission_mode ?? "B");
    const payment_type = String(req.body?.payment_type ?? "SALE");
...
router.post(
  ...
  async (req, res) => {
    try {
      const status = String(req.query.status || "0");
      ...
      return res
        .status(500)
        .send(
          errorPage((err as Error).message || "Failed to process transaction result."),
        );
    }
  },
);
...
function loadingPage(txtMtrId: string, txtAmount: string, meterInfo: Record<string, any> = {}): string {
